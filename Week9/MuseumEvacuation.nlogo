turtles-own [direction walldesire walltime visited nexttask]
...
to look-for-attraction
 ask turtles [
   let patches-in-cone patches in-cone 10 80

   let target-patches patches-in-cone with [pcolor = brown]
...
to lookforwall
  ifelse wall?
  [
    if havevisited? (list xcor ycor)
    [
      right 90
      stop
    ]

    ifelse walltime < walldesire
    [
     set walltime walltime + 1
    ]
    [
      ...
      set visited fput (list xcor ycor) visited
    ]
  ]
  [
      fd 1
